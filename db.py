...
def get(id, basename="mytable"):
    ...
    conn.execute(
        f"""CREATE TABLE IF NOT EXISTS {basename}
    ...
    if text_res:
        ...
        return Joke(
            id=id,
            text=text_res[0],
            likes=int(likes_res[0]),
            dislikes=int(dislikes_res[0]),
        )
    else:
        conn.close()
        return Joke()
...
def add(joke, basename="mytable"):
    ...
    conn.execute(
        f"INSERT INTO {basename} (id, text, likes, dislikes) VALUES (?, ?, ?, ?)",
        (joke.id, joke.text, joke.likes, joke.dislikes),
    )
    conn.commit()
